...
export default React.memo(({ orders, website }) => {
	...
	useEffect(() => {
		orders.forEach(order => {
			if (order.data[12].status === 'pending') setHasIncomplete(true)
		})
	},[]) //eslint-disable-line

	const checkIfIncomplete = (id) => {
		orders.forEach(order => {
			if (order.data[12].status === 'pending' && order.id !== parseInt(id)) setHasIncomplete(true)
		})
	}
	...
	return (
		<div className='orders'>
			...
			{orders.map((order, i) => {
				return (
					<div className='order' key={i} onClick={(e) => toggleShow(e)} style={{display: (order.data[12].status !== 'complete' || completed) ? 'block' : 'none'}}>
						<div className='top'>
						...
						<div className='main'>
							...
							<div className='product'>
								{order.data[11] && order.data[11].items && order.data[11].items.map((item, j) => {
									return (
